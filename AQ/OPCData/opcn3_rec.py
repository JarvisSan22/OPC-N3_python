...
class Opcn3:
        ...
        #function for all the hist data, to break up the getHist
            ...
            data['Bin 24'] = self.combine_bytes(ans[48],ans[49])
            data['period'] = self.combine_bytes(ans[52],ans[53])
            data['FlowRate'] = self.combine_bytes(ans[54],ans[55])
            data['Temp']=self.Tempcon(self.combine_bytes(ans[56],ans[57]))
            data['RH'] = self.RHcon(self.combine_bytes(ans[58],ans[59]))
                ...
                #    print("get hist attempt ",T)
                 ...
                 #  # print(nl)
                   ...
                   #     print("ans=",ans,"len",len(ans))
                    ...
                    if T > 20:
                     #   print("Reset SPI")
                        time.sleep(2) #time for spi buffer to reset
                        #reset SPI  conncetion 
                        self.initOPC(ser)
                        T=0
        ...
        def getData(self,OPCPORT,OPCNAME):
                ...
                try:
                    data=t
                    print(OPCNAME," Time",tnow , " PM1:", str(data['pm1']) ,"PM2:", str(data['pm2.5']) ,"PM10:", str(data['pm10']))
